...
export default function ProfileScreen({ route, navigation }) {
	...
	//load user info from props
	const userData =
		route && route.params ? route.params.userDataParam : undefined;
	...
	useEffect(() => {
		...
		if (!userData) {
			getDoc(doc(firestore, "users", auth.currentUser.uid))
				...
				.then(() => {
					getDownloadURL(ref(storage, tempUserInfo.picture)).then(
						(x) => {
							setImage(x);
						}
					);
			...
			getDownloadURL(ref(storage, tempUserInfo.picture))
				.then((x) => {
					setImage(x);
				})
				.catch((error) => {
	...
	const follow = () => {
		...
		updateDoc(ref, { followers: arrayUnion(auth.currentUser.uid) })
			.then(() => {
				updateDoc(doc(firestore, "users", auth.currentUser.uid), {
					following: arrayUnion(userInfo.id),
				});
			})
			.then(() => {
				setUserInfo({
					...userInfo,
					followers: [...userInfo.followers, auth.currentUser.uid],
				});
			})
			.catch((error) => {
				console.log(error);
	...
	return (
		<ScrollView>
			...
			<View
				style={Platform.OS == "ios" ? styles.menuIOS : styles.menuWeb}
			>
				<Text
					style={menu != 0 ? styles.menuText : styles.menuTextBold}
					onPress={changeMenu(0)}
				>
					Posts
				</Text>
				<Text
					style={menu != 1 ? styles.menuText : styles.menuTextBold}
					onPress={changeMenu(1)}
				>
					Workouts
				</Text>
				<Text
					style={menu != 2 ? styles.menuText : styles.menuTextBold}
					onPress={changeMenu(2)}
				>
					PRs
				</Text>
				<Text
					style={menu != 3 ? styles.menuText : styles.menuTextBold}
					onPress={changeMenu(3)}
				>
					Progress
				</Text>
			</View>
			<View>
				{menu == 0 &&
					(userInfo.id != 0 ? (
						<PostsFeed profilePage={true} userId={userInfo.id} />
					) : (
						<></>
					))}
				{menu == 1 && <WorkoutsPage />}
